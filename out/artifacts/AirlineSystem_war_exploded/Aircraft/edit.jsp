            ...
            <div class="col-md-7">
                <div class="card">
                    <div class="card-header">
                        <h3 class="header-title">Edit Aircraft Details</h3>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <%if(request.getMethod().equalsIgnoreCase("GET")) {
                            ...
                            aircraft = aircraftRepository.find(reg_No);

                        %>
                        <form method="post" >
                            <div class="form-group">
                                <label>Registration Number:</label>
                                <input type="text" class="form-control" placeholder="Reg_No" name="reg_No" value="<%=aircraft.getRegNo()%>" readonly/>
                            </div>

                            <div class="form-group">
                                <label>Model:</label>
                                <input type="text" placeholder="Model" class="form-control" name="type" value="<%=aircraft.getType()%>"/>
                            </div>

                            <div class="form-group">
                                <label>Name:</label>
                                <input type="text" class="form-control" placeholder="Aircraft Name" name="name" value="<%=aircraft.getName()%>"/>
                            </div>

                            <div class="form-group">
                                <label>Capacity:</label>
                                <input type="number" class="form-control" placeholder="Capacity" name="capacity" value="<%=aircraft.getCapacity()%>"/>
                            </div>

                            <button class="btn btn-lg btn-primary btn-block" type="submit">Edit Aircraft</button>
                        </form>
                        <%}%>
                    </div>
                </div>
...
<%
    if(request.getMethod().equalsIgnoreCase("POST")) {
        ...
        AircraftRepository aircraftRepository = new AircraftRepository();
        boolean result = aircraftRepository.updateAir(registrationNo, type, name, capacity);
        if(result){
            out.println(String.format("Congrats, you have successfull edit Aircraft with reg_No: %s", registrationNo));
            response.sendRedirect("list.jsp");}
        else
            out.println(String.format("Sorry, %s update failed", registrationNo));
    }
